...
export default {
  ...
  methods: {
    ...
    async startUpload() {
      // if (!this.allowUpload) {
      //   this.updateTips("warning", "正在读取文件计算哈希中，请耐心等待");
      //   return;
      // }
      this.chunkDoneTotal = 0;
      // 1.获取文件
      ...
      if (res && res.type == 2) {
        ...
        this.updateTips("warning", "分片正在加紧上传中，请勿关闭网页窗口");
        // 3.初始化对应请求队列并执行
        await this.buildFormDataToSend(
          this.chunkTotal,
          this.file,
          this.fileHash,
          res
    ...
    async postFileHash() {
      try {
        const res = await this.$api.hash_check({
          hash: this.fileHash,
        ...
        if (res.data.success) {
          ...
          this.updateTips("error", res.data);
          return res.data;
        }
      } catch (err) {
    ...
    async postChunkMerge(data) {
      const that = this;
      this.$api.chunks_merge(data).then((res) => {
        if (res.data.success) {
    ...
    async buildFormDataToSend(chunkCount, file, hash, res) {
      ...
      for (let i = 0; i < chunkCount; i++) {
        if (
          ...
          console.log(file, file.name, typeof(file.name));
          // 构建需要上传的分片数据
          ...
          const chunkReqItem = this.$api.chunks_upload(form, {
            onUploadProgress: (e) => {
