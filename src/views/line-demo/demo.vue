...
export default({
  ...
  // 离开页面时销毁所有line
  beforeDestroy(){
    setTimeout(this.removeLines, 500);
  },
  methods: {
    // 销毁线条
    removeLines() {
      if(this.lines && this.lines.length){
        this.lines.forEach((line, i) => {
          document.body.appendChild(this.elmLine[i]);
          line.remove()
        })
      }
    },
    drawLeaderLine() {
      this.$nextTick(()=> {
        ...
        this.elmLine = [elmLine1, elmLine2, elmLine3, elmLine4]
